...
def eval_fitness(generation, config):
    ...
    for gid, genome in generation:
        ...
        while True:
          ...
          # Change number of outputs to 2 in cofuguration file
          # Use output[1] to decide if forward will be true or false to acceralate and break the car, Also increment the fitness by 0.5 if car move forward and decrement the fitness by -0.5 so that car is not only standing still
          
          
             # If genome fitness reduces to -1 then break
